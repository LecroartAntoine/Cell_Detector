...
import os, cv2, psutil, sys, re, utils
import numpy as np
...
class DetectThread(QtCore.QThread):
    ...
    def __init__(self, images, taux_conf, box_width, show_conf, single = None, parent=None):
    ...
    def run(self):
        if self.single:
            ...
            self.images[self.single]['image_pred'] = utils.plot_bboxes(self.images[self.single]['image'], self.images[self.single]['pred'].boxes.boxes, self.box_width, self.show_conf)
            ...
            for key in self.images:
                ...
                self.images[key]['image_pred'] = utils.plot_bboxes(self.images[key]['image'], self.images[key]['pred'].boxes.boxes, self.box_width, self.show_conf)
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.analyse_viewer = PhotoViewer(MainWindow)
...
#####################################  div  ########################################################
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Détecteur de cellules"))
    ...
    def get_pred(self, single = False):
        ...
        if single:
            self.worker = DetectThread(self.images, self.taux_conf.value(), self.box_width.value(), self.show_conf.isChecked(), self.files.currentItem().text())
        else:
            self.worker = DetectThread(self.images, self.taux_conf.value(), self.box_width.value(), self.show_conf.isChecked())
